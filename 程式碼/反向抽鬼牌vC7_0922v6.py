...
# 四、博弈期第一手：明牌期第一位（持 10 分者）擲硬幣決定抽誰（原規則不變）；明牌期不動。〔2026/09/22 v5：他已在明牌期退出時，從原點的位置擲硬幣，見檔頭〕
...
#      （vC6.1：固定由座位第一位先抽）
#   2. 博弈期第一位抽牌者＝明牌期第一位；之後「被抽的人成為下一位抽牌者」〔2026/09/22 已更正：錨點應為抽牌者，見檔頭〕
...
class GameEngine:
    def __init__(self):
        ...
        self._last_target = None    # 本輪「被抽／被奪取」的玩家；只有發生保底時，被奪取者才接著抽（保底例外）
    ...
    def _check_passive_after_drawn(self, target):
        """被抽牌後檢查 target 狀態：只剩一張獎勵牌者當場退出；其餘設定 pending 標記，等 target 自己回合才處理。"""
        if target.exited:
            return
        if target.can_exit():
    ...
    def run_turn(self, drawer, fixed_target=None):
        ...
        if not drawer.exited and drawer.get_pairs()[0]:
            ...
            # [v6] 已選擇不清空時，這一手不再重複決定（手牌上限的強制部分照舊）
            if not (self._declined_clear and not drawer.must_discard()):
                drawer.discard_by_rule()
            if drawer.is_hand_empty():
                self._call_redistribution(drawer)
            if drawer.can_exit():
