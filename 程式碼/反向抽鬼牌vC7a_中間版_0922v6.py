# ============================================================
# 2026/09/22 v6（作者確認）：清不清空由玩家決定，主動與被動一致。
#   強制只有兩條：手牌上限（達 5 張丟對子到 5 張以下，單張不在此限）、坍縮後兩對以上清到剩一對。
#   手上全是對子、沒有獎勵牌時，主動達成也由玩家選擇清空或保留（AI 依與被動清空相同的傾向決定）。
#   選擇不清空的那一手，不再重複做一次丟對子的決定；手牌上限與坍縮的強制部分照舊。
# 2026/09/22 v5（作者確認）：博弈期從明牌期的原點（位置）開始。原點在明牌期已退出時，照退出規則：
#   從原點的位置擲硬幣決定下一位抽牌者，他再擲一次決定抽誰。重點是錨點（位置），不是 10 分在誰手上。
# 2026/09/22 v4（作者確認）：
#   一、重新接續時（有人退出或保底之後），硬幣選到的被抽者手上沒有牌（被動歸零、等待中）：
#       他當場保底並強制退出；抽牌者這一手不抽、不計一輪，由被奪取者接著抽（保底例外）。
#   二、保底時張數相同比牌面總分；總分也相同（例如 10＋2 對 8＋4），由保底者自己選，AI 一律隨機。
#   三、明牌期中途，被抽到只剩一張獎勵牌、抽完只剩一張獎勵牌、或保底後只剩一張獎勵牌者，當場退出（與博弈期一致）。
# 2026/09/22 v3（作者確認）：被動強制退出不需等待。
#   被抽到只剩一張獎勵牌 → 沒有選擇權，當場退出；下一位依退出規則，從他的位置擲硬幣決定
#   （坍縮後照輪轉方向），那個人再擲一次決定抽誰。
#   被動退出資格（有選擇權）與被動歸零（保底是他要去奪取的動作）仍等到自己抽牌的回合。
#   同一回合抽牌者也退出時，以抽牌者的退出為準（較晚發生）；有保底時仍由被奪取者接著抽。
# 2026/09/22 抽牌順序與接續規則修正（作者確認版）
# 一、一般抽牌：先抽後被抽。剛抽完、沒有退出的人成為下一個被抽的人，
#     硬幣決定由他哪一側的人來抽他；抽牌者抽的是自己 direction 側的鄰座，
#     所以來抽他的是他 -direction 側的鄰座。坍縮後方向鎖定＝固定輪轉。
# 二、有人退出（抽牌前退出、抽完後退出，非保底）：從退出者的位置擲硬幣決定下一位抽牌者，
#     該抽牌者再擲一次決定抽誰。坍縮後不擲硬幣，照輪轉方向（退出者 -direction 側）。
# 三、保底為例外：被奪取者接著抽（原規則不變），由他擲硬幣決定抽誰。
# 四、博弈期第一手：明牌期第一位（持 10 分者）擲硬幣決定抽誰（原規則不變）；明牌期不動。
# 反向抽鬼牌 vC7a － 修正一：抽牌順序（2026/09/17）
# 版本對照用，非最終版。最終版為 反向抽鬼牌vC7.py
#
# 與 vC6.1 的差異（只有這一項）：
#   1. 明牌期由持有 10 分獎勵牌者先抽，依方向繞一圈
#   2. 博弈期第一位抽牌者＝明牌期第一位
#   3. 被抽的人成為下一位抽牌者；每次抽牌前擲硬幣決定方向〔2026/09/22 已更正：錨點應為抽牌者，見檔頭〕
#   4. 被保底奪取者視同被抽，成為下一位抽牌者
#   5. 抽牌前退出者：擲硬幣決定下一位抽牌者
#
# 仍保留 vC6.1 的兩項錯誤實作：
#   - 丟對子與坍縮清對子，在「輪到自己、抽牌之前」處理
#   - 手牌達 5 張時清光所有對子
# ============================================================
...
class GameEngine:
    ...
    def redistribution(self, player):
        ...
        top = (len(candidates[0].get_reward_cards()), sum(c.points for c in candidates[0].get_reward_cards()))
        tied = [p for p in candidates
                if (len(p.get_reward_cards()), sum(c.points for c in p.get_reward_cards())) == top]
        # [v4] 張數與總分都相同：由保底者自己選，AI 一律隨機
    ...
    def _call_redistribution(self, player):
        ...
        for p in self.players:
            if p is not player and len(p.get_reward_cards()) < before[p.name]:
                self._last_target = p
                self._redist = True   # [2026/09/22] 本次發生保底

    def _draw(self, drawer, target, chosen):
        self._drew = True   # [2026/09/22]
    ...
    def open_round(self):
        ...
        for drawer in order:
            ...
            # [v4] 明牌期只剩一張獎勵牌者當場退出（被抽者、抽牌者、保底的被奪取者）
            for p in (target, drawer, self._last_target):
                if p is not None and not p.exited and p.can_exit():
                    self.do_exit(p, reason="forced_open_round_immediate")
    ...
    def _check_passive_after_drawn(self, target):
        ...
        if target.exited:
            return
        if target.can_exit():
            # [2026/09/22 v3] 被動強制退出不需等待：沒有選擇權，當場退出
            self.do_exit(target, reason="forced_passive_immediate")
            self._passive_exit = target
            print(f"  ✅ {target.name} 被抽到只剩一張獎勵牌，當場強制退出！得 {target.score} 分")
            return
    ...
    def run_turn(self, drawer, fixed_target=None):
        self.turn_count += 1

        if self.turn_count == COLLAPSE_TURN and not self.disaster_triggered:
        ...
        print(f"\n{'─' * 55}")
        print(f"🔄 第{self.turn_count}輪 | {drawer.name}({drawer.personality}) | 手牌{len(drawer.hand)}張")
        ...
        # === 抽牌 ===
        # [2026/09/22] 一般情況下被抽的對象固定為上一位抽牌者；沒有固定對象時才擲硬幣
        if (fixed_target is not None and not fixed_target.exited
                and fixed_target.hand and fixed_target is not drawer):
            target = fixed_target
        else:
        ...
        if target is not None and not target.exited and not target.hand:
            # [v4] 選到手上沒牌的人（被動歸零、等待中）：他當場保底並強制退出；
            # 抽牌者這一手不抽、不計一輪，由被奪取者接著抽（保底例外）
            self._call_redistribution(target)
            if target.can_exit():
                self.do_exit(target, reason="empty_target_early_redistribution")
            self._early_redist = True
            print(f"  🎯 {target.name} 手上沒牌卻被選為抽牌對象，當場保底並退出")
            return
        ...
        if not drawer.exited:
            ...
            if len(rewards) == 0 and len(pairs) > 0 and len(singles) == 0 and not drawer.want_to_discard():
                self._declined_clear = True   # [v6] 清不清空由玩家決定
                print(f"  🤔 {drawer.name} 主動達成清空條件，選擇保留對子")
    ...
    def run_game(self):
        ...
        if drawer is not None and drawer.exited:
            # [v5] 原點在明牌期已退出：照退出規則，從原點的位置擲硬幣決定下一位抽牌者
            if not self.disaster_triggered:
                self.direction = random.choice([1, -1])
            drawer = self._neighbor(drawer, -self.direction)
        if drawer is None or drawer.exited:
            drawer = next((p for p in self.players if not p.exited), None)
        anchor = None   # [2026/09/22] 下一個被抽的人
        self._passive_exit = None
        self._early_redist = False
        self._declined_clear = False
        ...
        while not self.check_game_end():
            ...
            self._last_target = None
            self._drew = False
            self._passive_exit = None   # [v3]
            self._early_redist = False  # [v4]
            self._declined_clear = False  # [v6]
            self._redist = False
            self.run_turn(drawer, fixed_target=anchor)
            anchor = None
            _snapshot(f"after_turn_{self.turn_count}", self)

            # [2026/09/22 接續規則修正]
            if drawer.exited and self._redist and self._last_target is not None \
                    and not self._last_target.exited:
                drawer = self._last_target          # 三、保底例外：被奪取者接著抽
            elif drawer.exited:
                # 二、非保底退出（抽牌前或抽完後）：從退出者位置擲硬幣；坍縮後照輪轉方向
                if not self.disaster_triggered:
                    self.direction = random.choice([1, -1])
                drawer = self._neighbor(drawer, -self.direction)
            elif self._early_redist and self._last_target is not None and not self._last_target.exited:
                drawer = self._last_target          # [v4] 空手者提前保底：被奪取者接著抽
            elif self._passive_exit is not None:
                # [v3] 被抽的人當場強制退出：從他的位置擲硬幣決定下一位；坍縮後照輪轉方向
                if not self.disaster_triggered:
                    self.direction = random.choice([1, -1])
                drawer = self._neighbor(self._passive_exit, -self.direction)
            elif self._drew:
                # 一、先抽後被抽：硬幣決定由他哪一側的人來抽他（來抽的人在他 -direction 側）
                if not self.disaster_triggered:
                    self.roll_direction()
                anchor = drawer
                drawer = self._neighbor(anchor, -self.direction)
            else:
                drawer = self._neighbor(drawer, self.direction)   # 找不到有效目標（原樣）
